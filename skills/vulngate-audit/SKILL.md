...
- Runtime-lab managed services must run under a recorded isolation backend. Linux prefers namespace/container backends; macOS requires a configured container/lightweight-VM backend. A repository boolean such as `allow_unconfined_start: true` is never sufficient: the operator must create a one-time approval bound to `run_id + config_digest + expiry`. If no backend or approval exists, fail closed. Record backend/version and both network/filesystem isolation contracts; external-ready services remain `unknown-external-process`.
