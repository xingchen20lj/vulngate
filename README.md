...
- **Python AST evidence** — S1 adds `semantic-ast-evidence-v1`: for Python files it confirms syntax-tree scope and branch membership, terminating guard shapes, alternate/exception paths, and parse gaps; it remains a bounded witness rather than a complete CFG or finding.
...
- **Language-aware value binding evidence** — S1 adds `semantic-python-binding-evidence-v1`: a bounded Python AST adapter explores simple assignments and finite branch/exception/loop paths, keeping raw-at-sink, mixed-path, derived, unresolved, and unsupported-language states explicit. It is still static research evidence, not SSA, type, runtime, or finding proof.
...
- Bounded control-flow relation evidence for guarded branches and alternate
  paths, with likely dominance kept as a review signal rather than proof.
...
|---|---|---|---|
| S1 | Attack-surface mapping, entry inventory, danger sites, fix history/variants, project profile, target rules, composite-chain, capability, semantic path/guard/call/control-flow/AST/transform/value-binding evidence, evidence provenance, and attacker-path threat-model views | `S1/entry-inventory.json`, `S1/security-fix-history.json`, `S1/patch-variants.json`, `S1/project-profile.json`, `S1/target-rules.json`, `S1/composite-chain-candidates.json`, `S1/capability-graph.json`, `S1/capability-candidates.json`, `S1/semantic-path-evidence.json`, `S1/semantic-path-candidates.json`, `S1/semantic-guard-evidence.json`, `S1/semantic-guard-candidates.json`, `S1/semantic-call-evidence.json`, `S1/semantic-call-candidates.json`, `S1/semantic-controlflow-evidence.json`, `S1/semantic-controlflow-candidates.json`, `S1/semantic-ast-evidence.json`, `S1/semantic-ast-candidates.json`, `S1/semantic-transform-evidence.json`, `S1/semantic-transform-candidates.json`, `S1/semantic-python-binding-evidence.json`, `S1/semantic-python-binding-candidates.json`, `S1/evidence-provenance.json`, `S1/threat-model.json` | G0 dead code, G1 reachability |
