...
| 36 | 有界跨符号调用点参数/返回绑定 | 已实现（含多边传播、预算和 Java parse-only facts） | `semantic-call-evidence-v2`、`semantic-call-evidence.json`、`semantic-call-candidates.json`、`semantic-calls` CLI | 对 bounded flow 的多条调用边绑定实参/形参，识别简单 `parameter -> local alias -> return`，记录 `returned`/`assigned` 与深度/节点/路径/时间预算；Java 仅 JDK parse，不做类型分析/类加载/目标执行；超预算显式 gap，静态线索保持 `not-a-finding`，不声称完整数据流或漏洞 |
...
| 38 | Python/Java AST 结构见证与解析缺口 | 已实现（Python AST + Java JDK parse-only structural evidence） | `semantic-ast-evidence-v2`、`semantic-ast-evidence.json`、`semantic-ast-candidates.json`、`semantic-ast` CLI | 以 Python AST/Java parse facts 确认作用域、终止守卫、`else`/异常备用路径和解析状态；不声称完整 CFG、SSA、类型/运行时证明，解析上限和其他语言保留显式降级 |
...
| 42 | 统一语法前端 | 已接入 Python/Java | `SemanticFrontend`、JDK parse-only adapter、既有 symbol/AST/binding/index summary | 实际消费者共用有界语法缓存；参数、嵌套符号、回退与解析缺口可回归，Java 不做类型分析/类加载/目标执行，语法置信度不升级为语义证明 |
